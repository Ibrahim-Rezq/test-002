...
const ShopBy = ({ filter, title }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted) {
                    console.error(
                        `Error while fetching products: ${err.message}`
                    )
                    setError(err)
                    setLoading(false)
                }
    ...
    return (
        <>
            ...
            <div
                className={`overflow-x-auto overflow-y-hidden md:max-w-full scroll-container mb-10 mx-auto relative scroll-container ${
                    filter === 'bestSellers' ? 'w-full' : ''
                }`}
            >
                {loading && <p>Loading...</p>}
                ...
                <div
                    className={`flex ${
                        filter === 'bestSellers'
                            ? 'flex-wrap justify-center m-auto'
                            : 'flex-nowrap space-x-4'
                    }`}
                >
                    {/* Ensure products is always an array */}
                    {filter === 'bestSellers'
                        ? copiedData.map((elem, id) => (
                              <div
                                  key={id}
                              ...
                              >
                                  <div className='absolute w-full flex justify-center items-center top-4  '>
                                      <p className='logo font-semibold z-50 text-sm'>
                                          {elem.name}
                                      </p>
                                  </div>
                                  <img
                             ...
                             bg-gray-800 text-white opacity-0 hover:opacity-80 transition-opacity duration-200 text-sm'
                                  >
                                      Explore →
                                  </button>
                              </div>
